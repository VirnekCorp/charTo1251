...
namespace VirnekCorp\charconvert;
 ...
 class CharConvert
{
    private static function charset_count_bad($s)
    ...
    private static function charset_count_chars($s)
    ...
    private  static function charset_count_pairs($s)
    ...
    private static function charset_alt_win($s)
    {
        for ($i = 0; $i < strlen($s); $i++) {
            $c = ord($s[$i]);
            if ($c >= 0x80 && $c <= 0x9f) {
    ...
    private static function charset_koi_win($s)
    {
        $kw = array(
    ...
    public  static function charset_utf8_win($s)
    {
        $r = '';
        $state = 1;
        for ($i = 0; $i < strlen($s); $i++) {
    ...
    private static function charset_prepare($s)
    {
        $r = 0;
        $k = 0;
...
// returns a string converted from a best encoding (windows-1251 or koi-8r) to windows-1251
        $sa = self::charset_prepare($s);
        $s1 = self::charset_win_lowercase($sa);
        $r1 = 'windows-1251';

        $c1 = self::charset_count_chars($s1);
        $b1 = self::charset_count_bad($s1);
        $p1 = self::charset_count_pairs($s1);
        $w1 = $p1 * 32 + $b1 * 64 - $c1;

        $s2 = self::charset_win_lowercase(self::charset_koi_win($sa));
        $w2 = -$c1; //Особенность кодировки koi-8r: тот же диапазон символов, что и для windows-1251
        if ($w2 < $w1) {
            $b2 = self::charset_count_bad($s2);
            $w2 += 64 * $b2;
            if ($w2 < $w1) {
                $p2 = self::charset_count_pairs($s2);
                $w2 += 32 * $p2;
                if ($w2 < $w1) {
                    $r1 = 'koi-8r';
                    $w1 = $w2;
        ...
        $s2 = self::charset_win_lowercase(self::charset_utf8_win($sa));

        $c2 = self::charset_count_chars($s2);
        $w2 = -$c2;
        if ($w2 < $w1) {
            $b2 = self::charset_count_bad($s2);
            $w2 += 64 * $b2;
        ...
        switch ($r1) {
            case 'alt':
                return self::charset_alt_win($s);
            case 'koi-8r':
                return self::charset_koi_win($s);
            case 'utf':
                return self::charset_utf8_win($s);
            default:
                return $s;
        }
